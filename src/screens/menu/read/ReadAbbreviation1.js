...
const ReadAbbreviation1 = ({ navigation }) => {
  const { speech } = useTTS();
  ...
  const menuList = [
    ...
    {'name': '학습하기', speech: () => speech('학습하기'), action: () => navigation.navigate('AbbreviationReader1')},
    {'name': '시험보기', speech: () => speech('시험보기'), action: () => navigation.navigate('AbbreviationRTester1')}
  ]
