...
const ReadInitialConsonant = ({ navigation }) => {
  const { speech } = useTTS();
  ...
  const menuList = [
    ...
    { name: '학습하기', speech: () => speech('학습하기'), action: () => navigation.navigate('InitialConsonantReader') },
    { name: '시험보기', speech: () => speech('시험보기'), action: () => navigation.navigate('InitialConsonantRTester') }
  ];
