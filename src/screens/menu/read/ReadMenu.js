...
const ReadMenu = ({ navigation }) => {
  ...
  const menuList = [
    ...
    { 'name': '튜토리얼', speech: () => speech('튜토리얼'), action: () => navigation.navigate('ReadTutorialMenu') },
    { 'name': '자음', speech: () => speech('자음'), action: () => navigation.navigate('ReadInitialConsonant') },
    { 'name': '모음', speech: () => speech('모음'), action: () => navigation.navigate('ReadVowel') },
    { 'name': '받침', speech: () => speech('받침'), action: () => navigation.navigate('ReadFinalConsonant') },
    { 'name': '약어 1단계', speech: () => speech('약어 1단계'), action: () => navigation.navigate('ReadAbbreviation1') },
    { 'name': '약어 2단계', speech: () => speech('약어 2단계'), action: () => navigation.navigate('ReadAbbreviation2') },
    { 'name': '약어 3단계', speech: () => speech('약어 3단계'), action: () => navigation.navigate('ReadAbbreviation3') },
    { 'name': '숫자', speech: () => speech('숫자'), action: () => navigation.navigate('ReadNumber') },
    { 'name': '영어(알파벳)', speech: () => speech('영어(알파벳)'), action: () => navigation.navigate('ReadAlphabet') },
  ]
  // 터치 이벤트 처리
